...
static int janus_mqttevh_send_message(void *context, const char *topic, json_t *message) {
	...
	/* The payload if generated by json_dumps needs to be freed */
	free(payload);
...
static void janus_mqttevh_client_connect_success(void *context, MQTTAsync_successData *response) {
	...
	int rc;
	if(ctx->will.enabled) {
		rc = janus_mqttevh_client_publish_message(ctx, topicbuf, ctx->will.retain, ctx->publish.connect_status);
	} else {
    	rc = janus_mqttevh_client_publish_message(ctx, topicbuf, ctx->publish.retain, ctx->publish.connect_status);
	}	
...
/* -- BW -- is this used at all? why is it disconnecting */
static int janus_mqttevh_client_reconnect(janus_mqttevh_context *ctx) {
...
static int janus_mqttevh_client_disconnect(janus_mqttevh_context *ctx) {
	...
	int rc;
	if(ctx->will.enabled) {
		rc = janus_mqttevh_client_publish_message(ctx, topicbuf, ctx->will.retain, ctx->publish.disconnect_status);
	} else {
    	rc = janus_mqttevh_client_publish_message(ctx, topicbuf, ctx->publish.retain, ctx->publish.disconnect_status);
	}
...
static int janus_mqttevh_client_publish_message(janus_mqttevh_context *ctx, const char *topic, int retain, char *payload)
{
	...
	if(rc == MQTTASYNC_SUCCESS) {
		JANUS_LOG(LOG_HUGE, "MQTT EVH message sent to topic %s on %s (QoS %d). Result %d\n", topic, ctx->connect.url, msg.qos, rc);
	} else {
		JANUS_LOG(LOG_WARN, "FAILURE: MQTT EVH message probably not sent to topic %s on %s (QoS %d). Result %d\n", topic, ctx->connect.url, msg.qos, rc);
	}
...
static int janus_mqttevh_init(const char *config_path) {
	...
	ctx->publish.qos = (qos_item && qos_item->value) ? atoi(qos_item->value) : DEFAULT_QOS;
